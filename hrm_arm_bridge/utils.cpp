...
void hrm::termostat_t::set_off(bool a_off)
{
  if (a_off) {
    mp_off_pin->set();
    //irs::mlog() << irsm("Òåðìîñòàò OFF") << endl;
  } else {
    mp_off_pin->clear();
    //irs::mlog() << irsm("Òåðìîñòàò ON") << endl;
  }
  m_status = irs_st_busy;
  m_timer.start();
}
...
void hrm::termostat_t::set_after_pause(counter_t a_after_pause)
{
  ...
  irs::mlog() << irsm("Òåðìîñòàò ïàóçà ") << CNT_TO_DBLTIME(a_after_pause);
  irs::mlog() << irsm(" ñ.") << endl;
}
...
void hrm::termostat_t::tick()
{
  if (m_timer.check()) {
    m_status = irs_st_ready;
    //irs::mlog() << irsm("Òåðìîñòàò ãîòîâ") << endl;
  }
}
