...
public:
  ...
  enum command_t {
    ...
    command_confirmation_stop_experiment,
    command_calculation_errors_dialog,
...
public:
  ...
  virtual irs::handle_t<form_t> make(
...
class settings_dialog_t: public form_t
{
public:
  settings_dialog_t(
    mxdisplay_drv_service_t* ap_lcd_drv_service,
...
private:
  menu_t();
  void show_prev_form();
  //void show_experiment_result(form_maker_base_t* ap_maker);
  //template <class form_type>
  //void reset();
  void reset(form_maker_base_t* ap_maker,
    form_maker_base_t *ap_parent_form_maker);
  void check_commands();
  void keep_current_form();
  mxdisplay_drv_service_t* mp_lcd_drv_service;
  mxkey_event_t* mp_menu_kb_event;
  eth_data_t* mp_eth_data;
  irs::handle_t<form_t> mp_form;
  //irs::handle_t<form_maker_base_t> mp_form_maker;
  form_maker_base_t* mp_form_maker;
  enum { form_makers_max_size = 5 };
  stack<irs::handle_t<form_maker_base_t> > m_form_makers;
  ...
  form_maker_t<settings_dialog_t> m_settings_dialog_maker;
};
...
{
  mp_form.reset();
  mp_form_maker.reset(new form_maker_t<form_type>());
